...
public class UserDataHandler
{
    ...
    public void PullLevelData()
    {
        ...
        Debug.Log("Got data of " + levelCount + " levels");
    }
    ...
    private int ReadLevelData()
    {
        ...
        try
        {
            ...
            {
                Debug.Log("Got nothing from the file loading up for the first time.");
                return -1;
            }

            JObject o1 = JObject.Parse(fileData);
            Debug.Log("Data read from file: " + o1.ToString());
            for(int i = 1; i <= o1.Count; ++i)
            {
                Debug.Log("Array Data: " + o1[i.ToString()].ToString());
        ...
        {
            Debug.LogError("Failed to read user data: " + ex.Message + "stacktrace: " + ex.StackTrace.ToString());
            size = -1;
        }
